...
class UARTCommunication:
    ...
    def _handshake(self) -> bool:
        ...
        try:
            ...
            if self.serial_conn.in_waiting > 0:
                ...
                # 尝试读取所有可用数据
                if self.serial_conn.in_waiting > 0:
                    all_data = self.serial_conn.read(self.serial_conn.in_waiting)
    ...
    def check_connection_status(self) -> dict:
        """检查连接状态"""
        time_since_handshake = time.time() - self.last_handshake_time if self.last_handshake_time > 0 else 0
...
def main():
    """主测试函数"""
    print("🚀 MaixCam与单片机UART通信测试 (独立版本)")
    print("=" * 60)
    print("📋 测试项目：")
    print("   1. 握手功能测试")
    print("   2. 数据传输测试")
